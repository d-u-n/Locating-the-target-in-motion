    ...
    if now - last_send_time >= SEND_INTERVAL:
        try:
            if latest_data["center_detected"]:
                # 当检测到中心点时，发送step和light值
                step = latest_data["step"][0]
                light = latest_data["light"][0]
                msg = f"y-step:{step:.2f}, 0.00, {light:.2f}\r\n"
            else:
                # 未检测到中心点时，只发送n-step
                msg = "n-step\r\n"
...
def detect_perspective_rectangle(frame):
    ...
    for cnt in contours:
        ...
        if len(approx) == 4:
            ...
            cv2.circle(vis, tuple(map(int, center)), 5, (0, 0, 255), -1)
            return center, vis, w
...
class TargetDetector:
    ...
    def process_frame(self, frame):
        ...
        # 绘制中心十字线
        cv2.line(vis, (img_w//2, 0), (img_w//2, img_h), (255,0,0), 1)
        cv2.line(vis, (0, img_h//2), (img_w, img_h//2), (255,0,0), 1)
        ...
        if self.state == State.INIT:
            ...
            self.last_state_change = time.time()
            cv2.putText(vis, "State: INIT -> YOLO_DETECTION", (10, 30), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,0,0), 2)
        
        elif self.state == State.YOLO_DETECTION:
            target_center, vis = self.yolo_detect(frame)
            
            if target_center:
                # 计算水平像素距离
                pixel_dist = abs(target_center[0] - image_center[0])
                cv2.putText(vis, f"YOLO Dist: {pixel_dist}", (10, 60), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,0,0), 2)
                ...
                if pixel_dist < 50:
                    ...
                    self.last_state_change = time.time()
                    cv2.putText(vis, "State: YOLO -> PRECISE", (10, 90), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,0,0), 2)
        
        elif self.state == State.PRECISE_ALIGNMENT:
            target_center, vis, w = detect_perspective_rectangle(frame)
            ...
            if target_center:
                ...
                # 计算step和light值
                step = (135/640)*(image_center[0]-target_center[0])
                light = 1.00 if pixel_dist < 5 else 0.00
                
                latest_data["step"] = [step]
                latest_data["light"] = [light]
                latest_data["center_detected"] = True
                
                cv2.putText(vis, f"Precise Dist: {pixel_dist}", (10, 60), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,0,0), 2)
                cv2.putText(vis, f"Distance: {distance_cm:.2f}cm", (10, 90), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,0,0), 2)
                
                if pixel_dist < 10:
                    self.state = State.COMPLETED
                    self.last_state_change = time.time()
                    cv2.putText(vis, "State: COMPLETED", (10, 120), 
            ...
            if target_center:
                ...
                cv2.putText(vis, f"Tracking Offset: {pixel_dist}px", (10, 60), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 2)
                cv2.putText(vis, f"Distance: {distance_cm:.2f}cm", (10, 90), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 2)
                cv2.putText(vis, "TRACKING MODE", (img_w//2-100, img_h//2), 
...
def main():
    ...
    try:
        # while not exit_program:
        #     # 等待开始指令
        #     while not program_running and not exit_program:
        #         time.sleep(0.1)
        #         continue
        while True:
            ret, frame = cap.read()
            ...
            vis, current_state = detector.process_frame(frame)
            cv2.imshow("Target Detection", vis)
